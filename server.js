...
5.5. **Generate [BASIC] Parameters (MANDATORY):** You MUST dynamically calculate and GENERATE the [BASIC] section at the end of the .VEH file. This high-level summary is critical for users to understand the setup's intent at a glance. It is NOT in the template; it must be fully derived.
...
- **Track-Specific Application:**
    ...
    - **Cold Tracks (Spa <15C):** You can gain "free" top speed. Use higher index values (e.g., 3-4) to close the ducts.
...
- **Springs (`SpringSetting`) & Packers (`PackerSetting`):**
    - **Springs:** Support the car under heavy aerodynamic load. Stiffer springs are mandatory on high-downforce tracks (Spa, Silverstone) to prevent the car from bottoming out at speed. Softer springs are used on bumpy tracks (Sebring) to improve compliance.
...
- **Brake Bias (`RearBrakeSetting`):** This value represents the percentage of brake force going to the rear. A higher value (e.g., 48%) moves the bias rearward, which helps the car rotate during trail-braking but can lead to rear locking and instability. A lower value (e.g., 45%) moves the bias forward, making the car more stable under braking but can cause front locking and entry understeer.
...
## CRITICAL KNOWLEDGE BASE (EXISTING)
// =====================================================================================
// --- START OF MASTERIZED KNOWLEDGE BASE (UNCHANGED CORE) ---
// =====================================================================================
// ... (The entire existing knowledge base from the prompt remains here, including HPU strategy, Track DNA, etc. The new sections above serve as targeted enhancements.)
// ... (For brevity, I am not re-printing the entire multi-thousand-line knowledge base that you provided, but it is assumed to be here)
