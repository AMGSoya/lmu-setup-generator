...
- **'Aggressive' Setup Goal:** Maximize *driveable* peak performance/responsiveness. NEVER compromise to an undrivable/unstable car. This means the car should be sharp, reactive, and consistently fast *in the hands of a skilled driver*.
    ...
    - **Anti-Roll Bars:** Stiffer front ARB to promote sharper turn-in (even if it means a touch more mid-corner understeer), and a slightly softer rear ARB to encourage rotation while maintaining predictability.
    - **Camber & Toe:** More negative camber (lower index) on the front for maximum cornering grip. Front toe-out (lower index) for aggressive turn-in. Rear toe-in (higher index) is still crucial for stability, but potentially a bit less than a 'Safe' setup to allow more rotation.
    ...
    - **Brakes:** Higher 'BrakePressureSetting' for maximum stopping power. 'RearBrakeSetting' might be slightly more rearward (higher index) to aid rotation on entry, but must remain controllable.
    ...
    - **[BASIC] Parameters:** 'Downforce' will be lower (0.05-0.30 depending on track). 'Balance' will be lower (0.15-0.35) indicating a more oversteer-prone, but drivable, car. 'Ride' will be lower (0.075-0.35) for a stiffer platform. 'Gearing' will be lower (0.075-0.45) for faster acceleration.
- **'Balanced' Setup Goal:** Optimize versatile, all-around performance for consistent lap times over a race stint. Strong compromise between stability and responsiveness. The car should be predictable, efficient, and easy to drive consistently, without being overly sluggish or nervous.
    ...
    - **Anti-Roll Bars:** Medium stiffness front and rear ARBs for a neutral mid-corner balance. Slight variations to fine-tune based on track or car.
    - **Camber & Toe:** Balanced camber settings to optimize tire contact patch through corners and on straights. Front toe-in (mid-range index) for stability, with rear toe-in (mid-range index) for predictability.
    ...
    - **Brakes:** Balanced 'BrakePressureSetting' for strong but manageable braking. Neutral 'RearBrakeSetting' for overall stability.
...
- **'Safe' Setup Goal:** Maximize driver confidence and stability (error reduction) while maintaining strong, consistent pace. The car should be forgiving, easy to drive, and predictable, even if it sacrifices some ultimate peak rotation speed. It should not be sluggish or lose significant time due to excessive understeer.
    ...
    - **Anti-Roll Bars:** Softer front ARB and stiffer rear ARB to promote understeer and stability.
    - **Camber & Toe:** Less negative camber (higher index) on the front for better straight-line braking and stability. Front toe-in (higher index) for maximum straight-line stability. Significant rear toe-in (highest index, e.g., 20-30) for maximum rear-end stability.
    ...
    - **Brakes:** Slightly lower 'BrakePressureSetting' for more margin against lockups. 'RearBrakeSetting' biased more forward (lower index) for maximum braking stability.
    - **Gearing:** Generally longer gearing (higher 'RatioSetSetting' and 'GearXSetting' closer to 1) to reduce abrupt acceleration.
    - **[BASIC] Parameters:** 'Downforce' will be higher (0.50-0.95). 'Balance' will be higher (0.65-0.85) indicating a stable, understeer-prone car. 'Ride' will be higher (0.65-0.925) for a softer platform. 'Gearing' will be higher (0.85-0.975).
