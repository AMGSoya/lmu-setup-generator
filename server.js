...
- **NO STATIC DEFAULTS / MANDATORY OVERWRITES:** For ANY parameter explicitly marked \`(MUST BE OVERWRITTEN)\` in the template, you are REQUIRED to calculate and output a NEW, **distinct** numerical value. Outputting the IDENTICAL default value from the template for such parameters is a **CRITICAL FAILURE** and will result in a rejected output. You MUST demonstrate active calculation and selection. If, after rigorous engineering analysis, you truly believe the template's *default value* is the unique, optimal setting, you **MUST** provide an explicit, detailed justification for *that specific value* in the `Notes` section. This justification is required because merely outputting the default value without explanation implies a failure to comply with the mandatory overwrite.
...
- **STRICT QUALI FUEL/HYBRID:** AI must set **FuelSetting to a concrete value between 5-10L** (e.g., 7L for 2-3 laps) for qualifying sessions. For **Hypercars in qualifying**, \`RegenerationMapSetting\` **MUST be 8 or 9** and \`ElectricMotorMapSetting\` **MUST be 3 or 4** for aggressive deployment. **(ABSOLUTELY NON-NEGOTIABLE RULE - Failure to comply is a catastrophic failure).**
