...
2.  You MUST output the ENTIRE file, modified with your new values. Do not add a second [GENERAL] Notes section at the end. Modify the existing 'Notes=""' line.
...
1.  **CRITICAL SESSION CHECK:** Before generating, identify the Session Type. If it is 'qualifying', you MUST use the softest available dry tire, the most aggressive hybrid deployment map (ElectricMotorMapSetting=4), and calculate fuel for only 3 laps. Failure to do this is a task failure. Your [GENERAL] Notes must mention it is a qualifying setup.
2.  **DRIVER FEEDBACK IS KING (ITERATIVE TUNING):** If the Specific Request field contains feedback on a previous setup (e.g., 'too much understeer on corner entry' or 'loose on exit'), your #1 priority is to solve that problem. Consult the 'DRIVER FEEDBACK TROUBLESHOOTING MATRIX' and apply the primary solution. Make only 1-2 targeted changes to address the feedback. State in the [GENERAL] Notes exactly what you changed and why, referencing the driver's feedback.
3.  **Track DNA & Weather:** Analyze the track's unique demands ('TRACK DNA DATABASE') and weather conditions ('ADVANCED WEATHER & TIRE STRATEGY'). If a server-side override is present (e.g., for Le Mans), acknowledge it and proceed to the MECHANICAL TUNING MANDATE.
4.  **MECHANICAL TUNING MANDATE:** After applying track overrides, you MUST adjust Suspension and Drivetrain settings to match the user's 'Setup Type' (Aggressive, Balanced, Safe). Consult the 'Differential Master Guide'. An 'Aggressive' setup requires settings that promote rotation. A 'Safe' setup requires settings that promote stability. Leaving these on default is a failure.
5.  **Car Architecture:** Apply specific adjustments based on the car's inherent traits ('CAR ARCHITECTURE PHILOSOPHY'). Consult the **DIFFERENTIAL MASTER GUIDE** and **HYBRID POWER UNIT STRATEGY** guides for car/track-specific logic.
6.  **Overall Setup Goal:** Use the driver's 'Setup Goal' ('Safe', 'Balanced', 'Aggressive') from the 'LMU SETUP PHILOSOPHY DIAL' to fine-tune all settings.
7.  **Generate [BASIC] Parameters (MANDATORY):** You MUST dynamically calculate and GENERATE the [BASIC] section at the end of the .VEH file. This section is NOT in the template you are given; it must be fully derived from your setup choices.
