...
// VERSION 2.1 UPDATE: Changed Hypercar RearAntiSwaySetting default from 0 (Detached) to 4 (a soft, but connected, setting) to prevent undrivable base setups.
...
RearAntiSwaySetting=4//P5 (Min: 0, Max: 20) (MUST BE OVERWRITTEN)
...
app.post('/generate-setup', async (req, res) => {
    ...
    if (track === "Circuit de la Sarthe (Le Mans)") {
        ...
        finalExampleTemplate = finalExampleTemplate.replace(/^(DRIVELINE[\s\S]*?FinalDriveSetting=)\d+/m, `$1${leMansFinalDrive}`);


        // Programmatically force ALL individual gears to 1 (Longest Ratio) AND RatioSetSetting to 1 (Long)
        finalExampleTemplate = finalExampleTemplate.replace(/^(Gear\dSetting=)\d+/gm, `$11`);
        finalExampleTemplate = finalExampleTemplate.replace(/^(RatioSetSetting=)\d+/m, `$11`);
        ...
        finalExampleTemplate = finalExampleTemplate.replace(/Notes=""/, `Notes="Le Mans override applied: Absolute minimum drag prioritized. FW/RW set to ${minAeroSetting}/${minAeroSetting}. [BASIC].Downforce will be extremely low. RatioSet and all individual gears set to Longest. FinalDrive set to ${leMansFinalDrive}. Radiators/Brake Ducts closed. Ride Heights minimized. This overrides general setup goals for max top speed."`);
    }
    ...
    if (track === "Autodromo Nazionale Monza") {
        ...
        finalExampleTemplate = finalExampleTemplate.replace(/^(DRIVELINE[\s\S]*?FinalDriveSetting=)\d+/m, `$1${monzaFinalDrive}`);


        finalExampleTemplate = finalExampleTemplate.replace(/^(Gear\dSetting=)\d+/gm, `$11`); // All individual gears to Longest (1)
        finalExampleTemplate = finalExampleTemplate.replace(/^(RatioSetSetting=)\d+/m, `$11`); // Force RatioSetSetting to 1 (Long)

        finalExampleTemplate = finalExampleTemplate.replace(/Notes=""/, `Notes="Monza override applied: Absolute minimum drag prioritized. FW/RW/Radiators/Ducts/RideHeights set to ${monzaMinAeroSetting}. [BASIC].Downforce will be extremely low. RatioSet and all individual gears set to Longest. FinalDrive set to ${monzaFinalDrive}. This overrides general setup goals for max top speed."`);
    }
...
1.  You will be given a complete .VEH file template below, which may have track-specific overrides already applied.
2.  You MUST output the ENTIRE file, modified with your new values. You must not output the [BASIC] section as it is not part of the template.
...
5.  Anti-Sway Bars MUST NOT be set to 0 unless the driver explicitly asks for a detached bar. A value of 0 is undrivable. Use a low number like 1 or 2 for a very soft setting.
...
## PRIME DIRECTIVE
Generate a complete, physically realistic, and numerically valid .VEH setup. Replace placeholders with calculated, logical numbers. Setting an adjustable parameter to its template default is a failure unless it is genuinely the optimal choice.
...
Populate '[GENERAL] Notes' with a concise engineering debrief. If a track-specific override (e.g., Le Mans aero) was applied, the notes will reflect this. You must ADD to these notes, explaining the logic behind your other key adjustments (e.g., 'Softened front fast dampers for better bump absorption at Sebring', 'Increased Diff Preload for better stability through Porsche Curves').

## THOUGHT PROCESS & HIERARCHY
1.  **Analyze Overrides:** First, check the '[GENERAL] Notes' in the template. If it contains an override message (e.g., for Le Mans, Monza, Sebring), those settings are locked in. Your subsequent decisions must work around these locked-in parameters.
2.  **Driver Feedback is KING:** Address 'Driver Problem to Solve' first. Consult 'DRIVER FEEDBACK TROUBLESHOOTING MATRIX' and the 'DRIVER REQUEST INTERPRETATION GUIDE'. Apply Primary/Secondary solutions. All other decisions must align with solving the driver's issue.
3.  **Track DNA & Weather:** Analyze the track's unique demands ('TRACK DNA DATABASE') and weather conditions. Apply baseline decisions and mention any necessary compromises in your notes.
4.  **Car Architecture:** Apply specific adjustments based on the car's inherent traits ('CAR ARCHITECTURE PHILOSOPHY').
5.  **Gearing Strategy:** Follow the 'GEARING STRATEGY (CRITICAL)' section. If a track override was not applied, you must decide on the complete gearing package based on the track's DNA.
6.  **Engineer's Debrief:** Add your concise summary to the 'Notes' section as per the 'CRITICAL INSTRUCTION'.

// =====================================================================================
// --- START OF LMU MASTER ENGINEERING KNOWLEDGE BASE ---
// =====================================================================================

## LMU SETUP FUNDAMENTALS (DEEP DIVE)
- **The Platform Concept:** The car's chassis is a 'platform'. All setup changes aim to keep this platform stable and predictable through all phases of a corner (entry, mid, exit). A stable platform allows the aerodynamics to work efficiently and gives the driver confidence. Key elements are controlling pitch (braking/acceleration), roll (cornering), and heave (vertical movement over bumps).
- **Pyramid of Grip:** Grip starts at the tires. 1. **Tires:** Must be kept in their optimal temperature and pressure window with the largest possible contact patch on the road. 2. **Suspension:** Manages the tire's contact with the road. It controls weight transfer and absorbs bumps. 3. **Aerodynamics:** Pushes the car onto the track, generating huge amounts of grip at speed. Aero is useless if the suspension can't keep the platform stable and the tires on the asphalt. All setup decisions must respect this hierarchy.
...
- **Aero Balance & Center of Pressure (CoP):** This is the point where the total aerodynamic force acts on the car. Moving the CoP forward (e.g., more front wing) increases front-end grip (can increase oversteer). Moving it rearward (e.g., more rear wing) increases rear-end grip (increases understeer and stability).
...
- **Camber Explained:** Negative camber is when the top of the tire is tilted inward. In a corner, the car rolls, and this negative camber helps to keep the outside tire's contact patch flat on the road, maximizing grip. Trade-off: Too much negative camber reduces the tire's contact patch during straight-line braking and acceleration.
- **Toe Explained:**
  - **Front Toe-out** (negative values) means the front of the tires point away from each other. This makes the car eager to turn-in but can cause instability on the straights.
  - **Rear Toe-in** (positive values) is CRITICAL for stability. It means the front of the rear tires point towards the centerline of the car. This provides significant stability under braking and on-throttle at corner exit.
- **Anti-Roll Bars (ARBs) Explained:** The ARB connects the left and right wheels on an axle to control how much the car's body rolls during cornering.
  - **Stiffening the front ARB** reduces front-end grip and increases understeer.
  - **Stiffening the rear ARB** reduces rear-end grip and increases oversteer. ARBs are a primary tool for adjusting mid-corner balance.
- **Dampers Explained:** Dampers control the *speed* of the suspension's movement.
  - **Bump:** Controls the suspension compressing.
    - **Slow-Speed Bump:** Manages slow body movements (roll, pitch). Stiffer slow bump = more support and responsiveness.
    - **Fast-Speed Bump:** Manages fast movements from bumps and curbs. Softer fast bump = better absorption.
  - **Rebound:** Controls the suspension extending.
    - **Slow-Speed Rebound:** Manages the body's return after a slow movement. Crucial for how the car "takes a set" in a corner.
    - **Fast-Speed Rebound:** Controls the wheel returning to the road after a bump. Must be set correctly to keep the tire in contact with the ground.

## DIFFERENTIAL DEEP DIVE
- The differential controls how much the driven wheels are "locked" together.
- **Power (Acceleration) Lock:** Controls locking on-throttle. More Lock (Higher Value) improves traction but can cause power-on understeer. Less Lock helps the car rotate but can cause inside wheelspin.
- **Coast (Deceleration) Lock:** Controls locking off-throttle/braking. More Lock (Higher Value) provides stability on corner entry but can cause entry understeer. Less Lock helps the car point into the corner but can make it unstable.
- **Preload:** A static amount of lock that is always present. It affects both entry and exit stability. More preload generally makes the car more stable and predictable, but also more prone to understeer.

## GEARING STRATEGY (CRITICAL)
- This is a holistic decision. You must set **FinalDriveSetting, RatioSetSetting, and ALL GearXSettings** together based on the track.
1.  **High-Speed Tracks (e.g., Le Mans, Monza):**
    - 'FinalDriveSetting': MUST be HIGHEST available index (e.g., 7 for Hypercar).
    - 'RatioSetSetting': MUST be 1 (Long).
    - 'Gear1Setting' through 'Gear7Setting': MUST ALL be 1 (Longest).
2.  **Technical/Accelerative Tracks (e.g., Sebring, Portimão):**
    - 'FinalDriveSetting': MUST be a LOW available index (e.g., 1-3 for Hypercar).
    - 'RatioSetSetting': MUST be 0 (Short).
    - 'Gear1Setting' through 'Gear7Setting': MUST ALL be 0 (Shortest).
3.  **Balanced Tracks (e.g., Spa, Fuji, Bahrain):**
    - Find a compromise. Use a mid-range 'FinalDriveSetting' (e.g., 4-5 for Hypercar).
    - 'RatioSetSetting' can be 0 or 1 depending on the most critical sector.
    - Individual gears MUST all be the same (all 0 or all 1) to match the 'RatioSetSetting'.

## TRACK DNA DATABASE (EXPANDED!)
- **Circuit de la Sarthe (Le Mans):** HIGH-SPEED. LOWEST DRAG. LONG GEARS. Server override forces this. Your job is to add stability for Porsche Curves via higher Diff Preload/Coast settings.
- **Sebring International Raceway:** EXTREMELY BUMPY. SOFT SUSPENSION & HIGH RIDE HEIGHT. Server override forces this. Your job is to tune for traction out of the hairpins using shorter gearing and higher diff power lock.
- **Spa-Francorchamps:** HIGH-SPEED, ELEVATION. Needs a stiff front platform (springs, slow bump) for Eau Rouge compression, but a car that rotates for the Bus Stop. Aero efficiency is key - enough wing for Pouhon without being a parachute on the Kemmel Straight. Use Balanced/Long Gearing.
- **Autodromo Nazionale Monza:** HIGHEST-SPEED. LOWEST DRAG. LONGEST GEARS. Server override forces this. Your job is to ensure braking stability (forward brake bias, high diff coast) and curb compliance (soft fast bump).
- **Fuji Speedway:** LONG STRAIGHT, TECHNICAL SECTOR 3. A classic compromise track. Low wing for Sector 1, but use mechanical grip (softer front ARB, aggressive diff settings) to get rotation in Sector 3. Balanced Gearing.
- **Autódromo Internacional do Algarve (Portimão):** "ROLLERCOASTER". Needs a predictable, confidence-inspiring platform for blind crests. Cannot be snappy. Use slightly higher ride heights and softer suspension. Shorter Gearing is generally preferred.
- **Bahrain International Circuit:** HIGH GRIP, HIGH WEAR. High rear tire temps are the main problem. Manage this with less negative rear camber and higher diff power lock to prevent wheelspin. Braking stability is key. Balanced Gearing.

## DRIVER REQUEST INTERPRETATION GUIDE
- IF driver asks for "oversteer", "rotation", "pointy": Interpret as AGGRESSIVE setup goal. The car must turn in sharply but remain PREDICTABLE on exit. It should feel 'on the edge' but never 'over the edge'. A snappy car is a failed setup.
- IF driver asks for "understeer", "stability", "safe": Interpret as SAFE setup goal. The car should be forgiving and confidence-inspiring.

## REAR PLATFORM STABILITY (MANDATORY PHILOSOPHY)
- A "loose" or "snappy" rear end is a failed setup, regardless of the user's request.
- **Rear Toe:** ALWAYS use a significant amount of Rear Toe-In for stability (value of 18-24 is a good starting point).
- **Differential Power Lock:** To prevent exit wheelspin/oversteer, use a relatively HIGH 'DiffPowerSetting' (e.g., 10-14 for Hypercar).
- **Rear Anti-Roll Bar:** Err on the side of a SOFTER 'RearAntiSwaySetting' (e.g. 2-6). A stiff rear ARB is a primary cause of snap oversteer. **Do not set to 0 (Detached).**

## CAR ARCHITECTURE PHILOSOPHY
- **Mid-Engine (Prototypes, Ferrari):** Inherently balanced. Flexible to tune.
- **Rear-Engine (Porsche):** Great traction, but entry/mid-corner understeer. Needs aggressive front-end settings (softer front ARB/springs) to make it turn.
- **Front-Engine (Corvette, Aston):** Great braking stability, but can have exit understeer/wheelspin. Needs settings that promote rear rotation (stiffer rear ARB, diff settings).

## DRIVER FEEDBACK TROUBLESHOOTING MATRIX
- "Oversteer on entry": Increase 'DiffCoastSetting', Stiffen 'FrontAntiSwaySetting'.
- "Oversteer on exit / Poor traction": Soften 'RearAntiSwaySetting', Increase 'DiffPowerSetting', reduce rear ride height (rake).
- "Understeer on entry": Reduce 'DiffCoastSetting', Soften 'FrontAntiSwaySetting'.
- "Understeer mid-corner/exit": Stiffen 'RearAntiSwaySetting', Reduce 'DiffPowerSetting', increase rear ride height (rake).
- "Unstable under braking": Move 'RearBrakeSetting' forward (lower number), Increase 'DiffCoastSetting'.

**FINAL COMMAND: The user's template is provided below. It might have server-side overrides applied. Copy it EXACTLY, only changing the numerical values as required by the engineering task and the rules above. Do not add, remove, or alter any lines or any comments. Do not output the [BASIC] section.**
    ...
    try {
        const openrouterResponse = await fetch(OPENROUTER_API_URL, {
            ...
            body: JSON.stringify({
                ...
                max_tokens: 4096, // Reduced slightly to stay within context limits
                temperature: 0.7, // Slightly reduced for more deterministic outputs
            }),
        });
        ...
        const basicSectionIndex = rawText.indexOf('[BASIC]');
        ...
        if (setupStartIndex !== -1) {
            ...
            // If a [BASIC] section was generated, truncate the string before it.
            if (basicSectionIndex > setupStartIndex) {
                setupText = rawText.substring(setupStartIndex, basicSectionIndex);
            }
            
            // Also remove any trailing markdown code blocks if they exist
            setupText = setupText.trim();
             if (setupText.endsWith('```')) {
                setupText = setupText.slice(0, -3).trim();
            }
            if (setupText.startsWith('```') && setupText.includes('```\n')) {
                 setupText = setupText.substring(setupText.indexOf('```\n') + 4);
            }
