// --- server.js (Final Version with AI-Centric Logic and Stability Fixes) ---
...
// 4. Configure CORS to allow your frontend to connect
// IMPORTANT: Replace with your actual frontend URL if you have one, otherwise '*' is for open access.
const corsOptions = {
    origin: '*' // Allows all origins. For production, you might restrict this to your frontend's domain.
};
...
// 6. Serve static files (if your backend also serves your frontend)
app.use(express.static(path.join(__dirname)));

// 7. Get your API keys from the .env file
const OPENROUTER_API_KEY = process.env.OPENROUTER_API_KEY;
const HUGGINGFACE_API_KEY = process.env.HUGGINGFACE_API_KEY; // Available if needed
...
[CONTROLS]
//SteerLockSetting=0//400 (16) deg (Fixed)
...
            const sectionRegex = new RegExp(`\\[<span class="math-inline">\{section\}\\\\\]\(\[\\\\s\\\\S\]\*?\)^\(</span>{settingName}=)\\d+(.*)`, 'm');
...
- **Rear Toe-In:** ALWAYS use a significant amount of Rear Toe-In for stability. For most cars and tracks, a value between 18 and 24 is a safe and effective starting point. NEVER use rear toe-out.
- **Differential Power Setting (DiffPowerSetting):** To prevent corner exit wheelspin and a loose rear, use a relatively HIGH 'DiffPowerSetting'. A higher value locks the differential more on-throttle, forcing both wheels to turn together and providing better traction. Start with higher values (e.g., 10-14 for Hypercar) and only reduce if the car has too much understeer on exit.
- **Rear Anti-Roll Bar (RearAntiSwaySetting):** Err on the side of a SOFTER rear anti-roll bar ('RearAntiSwaySetting'). A stiff rear ARB is a primary cause of snap oversteer. It is better to have a slightly softer rear and use other tools to manage rotation.
...
- **CRITICAL: Diff Settings: The AI MUST specify concrete, calculated values for DiffPowerSetting, DiffCoastSetting, and DiffPreloadSetting. Do NOT leave these as defaults or generic placeholders. These are crucial for handling.**
...
                }],
                max_tokens: 8192,
                temperature: 0.5, // Slightly increased temperature to encourage non-default generation, while relying on strict instructions.
            }),
        });
